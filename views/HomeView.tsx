...
// Only icons for quick links are needed now.
...
// Updated component to support the new glowing border effect and animated gradient background on hover.
const ImageCategoryCard: React.FC<{ title: string; imageUrl: string; gradientClass: string; onClick?: () => void }> = ({ title, imageUrl, gradientClass, onClick }) => (
    ...
    >
        {/* Container for the glowing effect. Increased rounding. */}
        <div className="glowing-border rounded-3xl p-[2px]">
            {/* Inner container holds the content and masks the border gradient. Increased rounding. */}
...
// The LearningCard component has been updated to show the entire image without cropping.
const LearningCard: React.FC<{ title: string; imageUrl: string; spanFull?: boolean; onClick?: () => void; }> = ({ title, imageUrl, spanFull, onClick }) => {
    ...
    // The main container is a rounded-3xl, overflow-hidden wrapper.
    ...
    return (
        <div onClick={onClick} className={`${spanFull ? 'col-span-2' : ''} ${cardBaseClasses}`}>
            {/* This div acts as the viewport for the image. It has a dark background that will be visible if the image's aspect ratio doesn't match the container's. */}
...
const HomeView: React.FC<HomeViewProps> = ({ onNavigate }) => {
    // The category data now includes a `gradientClass` for the new animated background effect.
    const categories = [
    ...
    // Data for the new Learning Library section with corrected, working image URLs.
    const learningItems = [
        { title: "Basics", imageUrl: "https://twiojujlmgannxhmrbou.supabase.co/storage/v1/object/public/app%20images/Gemini_Generated_Image_5q6l4w5q6l4w5q6l.png", spanFull: true, onClick: () => onNavigate('learningHome') },
        { title: "Bullish\nCandlesticks", imageUrl: "https://twiojujlmgannxhmrbou.supabase.co/storage/v1/object/public/app%20images/Gemini_Generated_Image_bcsd0zbcsd0zbcsd%20(1).png", spanFull: false, onClick: () => onNavigate('bullishPatternsList') },
        { title: "Bearish\nCandlesticks", imageUrl: "https://twiojujlmgannxhmrbou.supabase.co/storage/v1/object/public/app%20images/Gemini_Generated_Image_9awzps9awzps9awz.png", spanFull: false, onClick: () => onNavigate('bearishPatternsList') },
        ...
        <div className="bg-sky-50 dark:bg-slate-900 font-sans p-3 space-y-3">
            ...
            <div style={{ animationDelay: '0ms' }} className="animate-fadeInUp p-4 bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-gray-200 dark:border-slate-700">
                <div className="grid grid-cols-4 gap-4">
                    ...
                    />
                    {/* FIX: Added onClick handler to navigate to the practice view. */}
                    <IconLink 
